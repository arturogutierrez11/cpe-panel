...
import { getPromotionStats } from "@/src/application/promotions/get-promotion-stats";
...
export async function GET(request) {
  try {
    ...
    const data = await getPromotionStats({ token });
    ...
    if (error.status === 401) {
      ...
      try {
        const data = await getPromotionStats({ token: refreshed.accessToken });
